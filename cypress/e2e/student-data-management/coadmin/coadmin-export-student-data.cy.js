describe('Export Student Data Test', () => {
    ...
    it('Cek perilaku co-admin sekolah mengunduh data siswa yang terdaftar ke dalam bentuk dokumen .csv', () => {
        ...
        buttons.forEach(({ selector, icon, text, url }) => {
            ...
            cy.get('body', { timeout: 5000 }).then(($body) => {
                if ($body.text().includes('Export data siswa berhasil!')) {
                    cy.log('Export berhasil');
                } else if ($body.text().includes('Terjadi kesalahan saat export data siswa.')) {
                    cy.log('Export gagal');
                } else {
                    cy.log('Tidak ada toast export');
                }
            });
        });
    });
