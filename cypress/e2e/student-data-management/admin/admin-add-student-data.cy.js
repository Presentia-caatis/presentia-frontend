describe('Add Student Data Test', () => {
    ...
    it('Cek perilaku user melihat data siswa', () => {
        ...
        buttons.forEach(({ selector, icon, text, url }) => {
                                ...
                                cy.wrap($cell)
                                    ...
                                    .then((textValue) => {
                                        ...
                                        cy.wrap(null).then(() => {
                                            ...
                                            } else if (colIndex === 2) { // NIS
                                                expect(textValue).to.match(/^\d+$/);
                                            } else if (colIndex === 3) { // NISN
                                                expect(textValue).to.match(/^\d+$/);
                                            } else if (colIndex === 4) { // Jenis Kelamin
                                                expect(textValue).to.match(/^(Laki-Laki|Perempuan)$/);
                                            } else if (colIndex === 5) { // Kelas
                                                expect(textValue).to.match(/\S+/);
            ...
            cy.contains('.p-confirm-popup', 'Apakah Anda yakin ingin menambahkan siswa ini?')
                .should('be.visible');
            cy.contains('button', 'Ya').click();
            cy.contains('button', 'Save').should('have.class', 'p-button-loading');
            cy.contains('button', 'Save', { timeout: 10000 }).should('not.have.class', 'p-button-loading');

            cy.get('.p-dialog', { timeout: 10000 }).should('not.exist');

            cy.get('.p-toast', { timeout: 10000 }).should('be.visible');
