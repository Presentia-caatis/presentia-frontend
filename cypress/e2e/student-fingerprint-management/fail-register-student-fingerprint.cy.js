describe("Failed to Register Student Fingerprint Test", () => {
    const school = Cypress.env('schoolName');
    const roles = ['admin'];

    roles.forEach((role) => {
        it(`Cek perilaku ${role === 'admin' ? 'admin sekolah'
            : role} tidak dapat mendaftarkan sidik jari siswa`, () => {
                cy.loginAs(role);
                ...
                buttons.forEach(({ selector, icon, text, url }) => {
                    ...
                    cy.get('.p-toast-message')
                        ...
                        .and('contain.text', 'Pilih siswa, sidik jari, dan masukkan nomor mesin.')
                        .and('be.visible');
                });
            });
    });
