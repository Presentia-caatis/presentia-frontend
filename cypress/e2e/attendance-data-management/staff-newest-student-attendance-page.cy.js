describe('Staff newest Student Attendance Page Test', () => {
  const roles = ['staf'];
  //tc-47
  roles.forEach((role) => {
    it(`Cek perilaku ${role === 'staf' ? 'staf sekolah'
      : role
      } dapat melihat update data presensi siswa harian terbaru `, () => {
      cy.loginAs(role);
      cy.contains("Sekolah yang dikelola").should("be.visible");

      const buttons = [{
        selector: 'button.p-button-success',
        icon: '.pi.pi-sign-in',
        text: 'Daftar Presensi Hari Ini',
