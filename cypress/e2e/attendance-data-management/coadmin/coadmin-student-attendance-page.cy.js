describe('Student Attendance Page Test', () => {
  ...
  it('Cek perilaku coadmin sekolah mengakses halaman presensi', () => {
    ...
    buttons.forEach(({ selector, icon, text, url }) => {
      ...
      cy.get('button.p-button-primary')
        ...
        .within(() => {
          cy.get('span.pi-refresh', { timeout: 30000 }).should('exist');
        });
      ...
      cy.get('h5')
        ...
        .invoke('text')
        .should((text) => {
          expect(text).to.match(/^\d{2}\/\d{1,2}\/\d{4}$/);
        });
      ...
      cy.contains('Memuat data kehadiran...', { timeout: 30000 }).should('not.exist');
