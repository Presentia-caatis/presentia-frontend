describe("Select Class for Attendance Data Test", () => {
    ...
    it("Cek perilaku co-admin sekolah memilih kelas untuk menampilkan daftar presensi", () => {
        ...
        buttons.forEach(({ selector, icon, text, url }) => {
            ...
            filters.forEach((filter) => {
                cy.contains("h5", filter.label, { timeout: 15000 })
                    .should("be.visible")
                    .parent()
                    .within(() => {
                        if (filter.elements) {
            ...
            cy.contains("Memuat data kehadiran...", { timeout: 40000 }).should(
                "not.exist"
            );
            ...
            cy.get(".p-multiselect-panel")
                .contains(".p-multiselect-item", "XI PF 1")
                .should("exist")
                .click();
                ...
                if ($rows.length === 1) {
                    ...
                    cy.get("table tbody tr").each(($row) => {
                        cy.wrap($row)
                            ...
                            .invoke("text")
                            .then((kelasText) => {
                                expect(kelasText.trim()).to.equal("XI PF 1");
                            });
                    });
                }
            });
