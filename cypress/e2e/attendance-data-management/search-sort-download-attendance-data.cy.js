describe('Search, sort and download Attendance Data Test', () => {
    ...
    roles.forEach((role) => {
        it(`Cek perilaku ${role === 'staf' ? 'staf sekolah'
            : role === 'admin' ? 'admin sekolah'
                ...
                melakukan pencarian data presensi siswa menggunakan kolom pencarian dan dapat melakukan sorting data presensi siswa dan dapat menunduh daftar presensi `, () => {
            cy.loginAs(role);
            ...
            buttons.forEach(({
                ...
                const filters = [{
                    ...
                    {
                        label: 'Pilih Jenis Kehadiran / Ketidakhadiran',
                        selector: '.p-multiselect'
                    }
                ];
                    ...
                    if ($rows.length === 1) {
                        ...
                        cy.get('table tbody tr').each(($row) => {
                            cy.wrap($row).find('td').eq(1)
                                .invoke('text')
                                .then((kelasText) => {
                                    expect(kelasText.trim()).to.equal('Budi');
                                });
                        });
                    }
                });
                ...
                cy.contains('.p-dialog .p-dialog-header', 'Export Data Kehadiran').should('be.visible');
